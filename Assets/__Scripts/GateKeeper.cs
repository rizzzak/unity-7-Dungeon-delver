...
{
    //  Константы зависят от файла изображения DelverTiles
    ...
    {
        //Если ключей нет - выход
        if (keys.keyCount < 1) return;

        Tile ti = collision.gameObject.GetComponent<Tile>();
        if (ti == null) return;

        //открытие только если Дрей смотрит на дверь
        int facing = keys.GetFacing();
        Tile ti2; //верхняя часть вертикальной двери(сост. из 2 плиток)
        switch (ti.tileNum)
        {
            case lockedR:
                if (facing != 0) return;
