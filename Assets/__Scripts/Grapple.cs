...
{
    //состояния крюка: неактивный, брошен, не попал, попал(притягиваемся)
    ...
    /// <summary>
    /// При нажатии клавиши броска X - инициализация броска(StartGrapple())
    /// </summary>
    void Update()
    {
    ...
    /// <summary>
    /// Определение нач. и кон. точек броска, изменение анимации, отключение User Control, Colld
    /// </summary>
    void StartGrapple()
    {
    ...
    private void FixedUpdate()
    {
        switch (mode)
        {
            // Крюк летит вперед до первой плитки, за которую можно зацепиться, оставяляя след
            // Если не зацепился - переход в состояние gInMiss
            case eMode.gOut:
                ...
                if ((p1 - p0).magnitude >= grappleLength)
                    mode = eMode.gInMiss;
                break;
            // При промахе - крюк возвращается на удвоенной скорости назад.
            case eMode.gInMiss:
                ...
                if (Vector3.Dot((p1 - p0), directions[facing]) > 0)
                {
                    ...
                    StopGrapple();
                break;
            // При попадании - Дрей начинает притягиваться к точке попадания
            case eMode.gInHit:
    ...
    /// <summary>
    /// Окончение притяжения: возвращается UserControls, Colld Дрею.
    /// Если оказались на небезопасной клетке - -хп, перенос к началу комнаты
    /// </summary>
    void StopGrapple()
    {
        dray.enabled = true;
    ...
    /// <summary>
    /// При попадании по врагу, притянуть крюк обратно
    /// </summary>
